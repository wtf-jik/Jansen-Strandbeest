...
% 'filename'                : Name of animation file created. (string)
...
args = struct('maxiterations', 10,'tolerance', 1e-10, 'resolution', 1e-5,...
              ...
              DEFAULT_T, 'linklengths', DEFAULT_L, 'filename', 'jansen');
...
for i = 1: length(crank)
    ...
    if args.creategif == true
        ...
        if i == 1 
          imwrite(imind,cm,sprintf('%s.gif', args.filename),'gif','Loopcount',inf); 
        else 
          imwrite(imind,cm,sprintf('%s.gif', args.filename),'gif','WriteMode','append','DelayTime', 0); 
        end
    end
end